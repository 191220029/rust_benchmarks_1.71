...
mod tests {
    ...
    #[test]
    fn test_item_append_param() {
        let mut buf = Vec::new();

        let item = Item::UInt(42);
        item.append_param(&mut buf);

        buf.clear();
        let item = Item::SInt(-42);
        item.append_param(&mut buf);

        buf.clear();
        let item = Item::Float(3.14);
        item.append_param(&mut buf);

        buf.clear();
        let item = Item::String("hello".into());
        item.append_param(&mut buf);

        buf.clear();
        let item = Item::Bin(vec![1, 2, 3, 4]);
        item.append_param(&mut buf);
    }

    #[test]
    fn test_parameterizer_new() {
        let p = Parameterizer::new("test query".into());
        assert_eq!(p.buf, b"test query");
        assert_eq!(p.i, 0);
    ...
    #[test]
    fn test_parameterizer_read_string() {
        let mut p = Parameterizer::new("\"hello\"".into());
        p.read_string(b'"').unwrap();
        assert_eq!(p.params, vec![Item::String("".into())]);
    }

    #[test]
    fn test_parameterizer_read_ident() {
        let mut p = Parameterizer::new("identifier".into());
        p.read_ident().unwrap();
        assert_eq!(p.query, b"identifier");
    }

    #[test]
    fn test_parameterizer_read_signed_integer() {
        let mut p = Parameterizer::new("-42".into());
        assert!(p.read_signed_integer().is_err());
    }

    #[test]
    fn test_parameterizer_read_unsigned_integer() {
        let mut p = Parameterizer::new("42".into());
        p.read_unsigned_integer().unwrap();
        assert_eq!(p.params, vec![Item::UInt(42)]);
    }

    #[test]
    fn test_parameterizer_read_binary() {
        let mut p = Parameterizer::new("`binary_data`".into());
        p.read_binary().unwrap();
    }

    #[test]
    fn test_parameterizer_not_eof() {
        let _ = Parameterizer::new("data".into());
        let _ = Parameterizer::new("".into());
    }

    #[test]
    fn test_connect_tcp() {
        let cfg = ClientConfig {
            kind: ClientConfigKind::Tcp("localhost".into(), 2003),
            username: "user".into(),
            password: "password".into(),
        };

        let tcp_called = Arc::new(Mutex::new(false));
        let tls_called = Arc::new(Mutex::new(false));

        let tcp_f = {
            let tcp_called = Arc::clone(&tcp_called);
            move |_: Connection| {
                *tcp_called.lock().unwrap() = true;
                Ok(())
            }
        };
        let tls_f = {
            let tls_called = Arc::clone(&tls_called);
            move |_: ConnectionTls| {
                *tls_called.lock().unwrap() = true;
                Ok(())
            }
        };

        assert!(connect(cfg, false, tcp_f, tls_f).is_err());
    }

    #[test]
    fn test_connect_tls() {
        let cfg = ClientConfig {
            kind: ClientConfigKind::Tls("localhost".into(), 2003, "cert".into()),
            username: "user".into(),
            password: "password".into(),
        };
