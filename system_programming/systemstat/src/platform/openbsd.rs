...
use crate::data::*;
use libc::{
    c_int, c_uchar, c_uint, c_ulong, c_void, freeifaddrs, getifaddrs, if_data, ifaddrs, ioctl,
    statfs, sysctl, timeval,
};
use std::mem::size_of;
...
impl Platform for PlatformImpl {
    ...
    fn boot_time(&self) -> io::Result<OffsetDateTime> {
        ...
        let ts = OffsetDateTime::from_unix_timestamp(data.tv_sec.into())
            .expect("unix timestamp should be within range")
            + Duration::from_nanos(data.tv_usec as u64);
        Ok(ts)
    }
...
fn statfs_to_fs(fs: &statfs) -> Filesystem {
    Filesystem {
        ...
        fs_type: unsafe {
            ffi::CStr::from_ptr(&fs.f_fstypename[0])
                .to_string_lossy()
                .into_owned()
        },
        fs_mounted_from: unsafe {
            ffi::CStr::from_ptr(&fs.f_mntfromname[0])
                .to_string_lossy()
                .into_owned()
        },
        fs_mounted_on: unsafe {
            ffi::CStr::from_ptr(&fs.f_mntonname[0])
                .to_string_lossy()
                .into_owned()
        },
    }
